...
Straight through 1 → 12 is correct. One optimisation if XSGD funding looks doubtful: build only
`wallet.ts` from Task 6, jump to Tasks 7, 9, 10, 11, and come back for `StraitsXIssuer`. That
reaches the offline end-to-end proof — spec success criterion 1, the thing you can actually
demo — hours earlier, and leaves the real-rail work as a clean bolt-on. `ISSUER=mock` is the
default everywhere, so nothing else changes.

Task 12 is not code and runs only once funding has landed.
...
**Files:**
- Create: `packages/pay/package.json`, `packages/pay/tsconfig.json`, `packages/pay/src/config.ts`, `packages/pay/src/db.ts`, `packages/pay/src/audit.ts`
- Modify: `.env.example` and `apps/api/src/env.ts` at the repo root (add the new keys, retire the old ones — see Step 0)
...
- [ ] **Step 0: Create the package and reconcile the env vocabulary**

`packages/pay/package.json` — note the caret ranges. This machine quarantines packages published in the last ~7 days, so only the crypto pins are exact.
...
Install: `pnpm install` from the repo root. If any package is rejected for being too recently published, widen the range rather than pinning.

**Then reconcile the env vocabulary.** The scaffold's `.env.example` and `apps/api/src/env.ts` already define `MIN_CARD_SGD`, `MAX_CARD_SGD`, `SLIPPAGE_BPS` and `AGENT_PRIVATE_KEY` for the same quantities this library calls `MIN_CARD_CENTS`, `MAX_CARD_CENTS`, `PRICE_TOLERANCE_BPS` and `SPEND_PRIVATE_KEY`. Two vocabularies over one root `.env` will drift, and a drifted rail bound is a declined card. The cents names win — they keep floats out of config entirely, the same reason `Minor` is a string in the shared schemas.

In `.env.example` and `apps/api/src/env.ts`: delete the four old keys and add all of these, wrapping every optional one in the existing `optional()` helper at the top of `env.ts` (it maps `""` to `undefined`, so a blank line in `.env` does not fail its own format regex):
...
Verify: `pnpm typecheck` passes across all packages and `pnpm --filter @happy/api dev` still boots.
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  if (!Number.isFinite(chainId)) throw new Error(`invalid CHAIN_ID: ${JSON.stringify(chainIdRaw)} is not a number`);
  if (!Number.isInteger(chainId)) throw new Error(`invalid CHAIN_ID: ${JSON.stringify(chainIdRaw)} must be an integer`);
...
const isForbiddenKey = (key: string): boolean => FORBIDDEN.test(key.replace(/_/g, '').toLowerCase());
...
Run: `pnpm --filter @happy/pay test rules`
Expected: PASS, 13 tests (14 assertions — the own-reservation case asserts both directions).
...
**Interfaces:**
- Produces: `IssuerAdapter` with **`prepare(req): Promise<PreparedPayment>`**, **`send(req, prepared): Promise<IssueResult>`** and `reveal(opaqueId: string): Promise<CardMaterial>`; `IssueRequest = { amountCents, cardholderName, idempotencyKey }`; `PreparedPayment = { nonce, envelope, validBeforeMs }`; `IssueResult = { opaqueId, last4, expiresAt, settlementTx }`; `CardMaterial = { pan, expiry, cvc }`.

**Why two calls instead of one.** The signed nonce and the exact envelope bytes must be on disk *before* anything is sent, or a lost response leaves a payment that reconciliation cannot identify on-chain and will wrongly write off as failed. `prepare` costs nothing and moves no money; `send` is the irreversible half. Replaying a `send` with the same prepared envelope is safe — the nonce is single-use on-chain, so a duplicate can never settle twice.
...
**Interfaces:**
- Consumes: `Config`.
- Produces: `TokenBucket`; `parseChallenge(res: Response, bodyText: string): ChallengeEntry`; `validateChallenge(entry, cfg, amountCents): void`; `newNonce(): 0x${string}`; `buildAuthorization(entry, from, nonce, nowSec?): Authorization`; `buildEnvelope(entry, signature, authorization): string`; `eip712(entry, authorization)` returning the viem `signTypedData` argument; `X402Error` with `code: 'RATE_LIMITED' | 'BAD_CHALLENGE' | 'REJECTED' | 'UNAVAILABLE'`.
...
- Produces: `makeWallet(cfg): Wallet` with `{ account, address, refresh(): Promise<Cents>, view(): ChainView, authorizationUsed(nonce): Promise<boolean>, start(), stop() }`; `StraitsXIssuer implements IssuerAdapter`.

> **Ordering note.** The `StraitsXIssuer` half of this task is dead weight if XSGD funding never arrives, and the offline demo (Tasks 7, 9, 10, 11) does not need it. `makeWallet` *is* needed early, because `index.ts` imports it. If funding looks doubtful, build `wallet.ts` here, skip to Task 7, and come back for the issuer once the rail is usable. The `straitsx` branch in `index.ts` can throw until then — `ISSUER=mock` is the default everywhere.
...
const entry = {
  scheme: 'exact', network: 'eip155:43113', amount: '18000000',
  asset: '0xd769410dc8772695a7f55a304d2125320a65c2a5',
  payTo: '0x99a2B2962a6AC463FBe04664027Fdb3F68bd4Cc8',
  maxTimeoutSeconds: 300, chainId: 43113,
  extra: { assetTransferMethod: 'eip3009', name: 'XSGD', version: '2' },
...
describe('StraitsXIssuer', () => {
  ...
  it('sends fractional SGD for cent amounts', async () => {
    ...
    const fetchImpl = vi.fn(async (_u: string, init: any) => {
      ...
      if (!init.headers['PAYMENT-SIGNATURE']) {
        return new Response(JSON.stringify({ accepts: [{ ...entry, amount: '18500000' }] }), { status: 402 });
      }
  ...
  it('maps HTTP 429 to a rate-limit error without parsing the body as JSON', async () => {
    ...
    const iss = new StraitsXIssuer(cfg, account, new TokenBucket(10, 60_000), fetchImpl as any);
    await expect(iss.prepare({ amountCents: 1800, cardholderName: 'Happy Agent', idempotencyKey: 'p3' }))
  ...
  it('refuses locally once the token bucket is empty, making no request', async () => {
    ...
    await expect(iss.prepare({ amountCents: 1800, cardholderName: 'Happy Agent', idempotencyKey: 'p4' }))
      .rejects.toMatchObject({ code: 'RATE_LIMITED' });
  ...
  it('replays a stored envelope instead of signing a new nonce', async () => {
    ...
    const fetchImpl = vi.fn(async (_u: string, init: any) =>
      init.headers['PAYMENT-SIGNATURE']
        ...
        : challengeRes());
    const iss = new StraitsXIssuer(cfg, account, new TokenBucket(10, 60_000), fetchImpl as any);
    const stored = { nonce: `0x${'ab'.repeat(32)}` as const, envelope: 'STORED_ENVELOPE', validBeforeMs: Date.now() + 300_000 };
    const r = await iss.send({ amountCents: 1800, cardholderName: 'Happy Agent', idempotencyKey: 'p5' }, stored);
...
describe('issueCardFor', () => {
  ...
  it('leaves the purchase PAYING and the payment PENDING when the issuer throws mid-flight', async () => {
    const p = await reserve();
    const issuer = new MockIssuer();
    vi.spyOn(issuer, 'send').mockRejectedValueOnce(Object.assign(new Error('socket hang up'), { code: 'UNAVAILABLE' }));
    const deps = { db, cfg, issuer, wallet };
    await expect(issueCardFor(deps, p.id, 1800)).rejects.toThrow(/socket hang up/);
    const row = db.raw.prepare(`SELECT state FROM purchases WHERE id=?`).get(p.id) as any;
    const pay = db.raw.prepare(`SELECT state, nonce, envelope FROM payments WHERE purchase_id=?`).get(p.id) as any;
...
export async function issueCardFor(deps: Deps, purchaseId: string, finalTotalCents: Cents): Promise<IssueResult> {
  ...
  if (existingCard) {
    return {
      opaqueId: existingCard.opaque_id, last4: existingCard.last4,
      expiresAt: existingCard.expires_at, settlementTx: existingPayment?.tx_hash ?? null,
    };
  }
  ...
  const req = { amountCents: faceCents, cardholderName: cfg.cardholderName, idempotencyKey: purchaseId };
  ...
  db.tx((t) => {
    ...
    t.raw.prepare(`INSERT INTO payments (nonce, purchase_id, amount_cents, valid_before, envelope, state, tx_hash, created_at)
                   VALUES (?,?,?,?,?,?,?,?)`)
      .run(prepared.nonce, purchaseId, faceCents, new Date(prepared.validBeforeMs).toISOString(),
           prepared.envelope, 'PENDING', null, iso());
  });
  ...
  db.tx((t) => {
    t.raw.prepare(`UPDATE payments SET state='SETTLED', tx_hash=? WHERE purchase_id=?`)
...
describe('resolvePending', () => {
  ...
  it('recovers the card by replaying the stored envelope when one is available', async () => {
    ...
    const issuer = {
      ...
      prepare: async () => { throw new Error('must not prepare a new nonce'); },
      send: async () => ({ opaqueId: 'card_recovered', last4: '4242', expiresAt: null, settlementTx: '0xtx' }),
      reveal: async () => { throw new Error('not needed'); },
    };
    ...
    if (used) {
      ...
      if (deps.issuer && p.envelope) {
        recovered = await deps.issuer
          .send({ amountCents: p.amount_cents, cardholderName: deps.cfg.cardholderName, idempotencyKey: p.purchase_id },
                { nonce: p.nonce, envelope: p.envelope, validBeforeMs: Date.parse(p.valid_before) })
          .catch(() => null);
      }
      db.tx((t) => {
        t.raw.prepare(`UPDATE payments SET state='SETTLED', tx_hash=? WHERE nonce=?`)
          .run(recovered?.settlementTx ?? null, p.nonce);
        if (recovered) {
          t.raw.prepare(`UPDATE purchases SET state='CARD_ISSUED', final_cents=?, updated_at=? WHERE id=?`)
            .run(p.amount_cents, iso(), p.purchase_id);
          t.raw.prepare(`INSERT OR REPLACE INTO cards VALUES (?,?,?,?,?,?,?)`)
            .run(p.purchase_id, deps.issuer!.name, recovered.opaqueId, recovered.last4, recovered.expiresAt, 'ACTIVE', iso());
          appendAudit(t, { purchaseId: p.purchase_id, kind: 'CARD_ISSUED',
...
**Files:**
- Create: `apps/demo-store/src/app.ts` — the Hono app, exported, with **no** top-level `serve()`
- Modify: `apps/demo-store/src/index.ts` — the scaffold's existing entrypoint; it already calls `serve()` on `DEMO_STORE_PORT`. Make it import `app` from `./app.js` instead of building its own, so `pnpm dev` serves the storefront rather than the health-only stub.
- Modify: `apps/demo-store/package.json` — add `"exports": { ".": "./src/index.ts", "./app": "./src/app.ts" }` so tests in other packages can import the app without reaching across the filesystem
- Test: `apps/demo-store/test/shop.test.ts`

> The scaffold created `src/index.ts`, not `src/server.ts`, and it is not empty — it builds a Hono app and calls `serve()` at module scope. Adding a second file that also builds an app leaves two apps in one package and `pnpm dev` still serving the wrong one. Split it: `app.ts` owns the routes, `index.ts` owns the listening.
...
Run: `pnpm --filter @happy/demo-store test`
Expected: PASS, 4 tests.
Then `pnpm dev` and confirm `curl localhost:4030/item/usb-c-hub` returns the product page, not the health stub.
...
Uses Playwright against the demo store, started in-process. Import it through the workspace
(`@happy/demo-store/app`), never by relative path — reaching across packages escapes
`rootDir` and breaks under pnpm's isolated `node_modules`.
...
export async function payWithCard(deps: CheckoutDeps, page: Page, purchaseId: string): Promise<CheckoutResult> {
  ...
  const ref = await page.locator('[data-order-ref]').first()
    .getAttribute('data-order-ref', { timeout: 2_000 }).catch(() => null);
  if (ref) {
...
Run: `pnpm --filter @happy/pay test checkout`
Expected: PASS, 3 tests. If Playwright browsers are missing, run `pnpm exec playwright install chromium` first.
...
function get(): Ctx {
  ...
  if (cfg.issuer === 'mock') {
    ...
    issuer = new StraitsXIssuer(cfg, wallet.account, new TokenBucket(cfg.railBucketCapacity, cfg.railBucketRefillMs));
  }
  wallet.start();
...
export async function getMandate() {
  ...
  return {
    ...
    footer: `Mandate active · auto-approve under ${display(m.per_item_cents)}/item · `
