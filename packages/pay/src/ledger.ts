...
import { type Decision, decide, type Reason } from "./rules.js";
...
export function getMandateRow(db: Db) {
  return db.raw
    .prepare(`SELECT * FROM mandates WHERE status='ACTIVE' ORDER BY created_at DESC LIMIT 1`)
    .get() as any;
}

export function totals(db: Db, mandateId: string | null) {
  if (!mandateId)
    return { spentCents: 0 as Cents, reservedCents: 0 as Cents, strandedCents: 0 as Cents };
  const r = db.raw
    .prepare(`
    SELECT
    ...
    FROM purchases WHERE mandate_id = ?`)
    .get(mandateId) as any;
  return {
    spentCents: r.spent as Cents,
    reservedCents: r.reserved as Cents,
    strandedCents: r.stranded as Cents,
  };
}
...
export async function createMandate(
  ...
  db.tx((t) => {
    ...
    t.raw
      .prepare(`INSERT INTO mandates VALUES (?,?,?,?,?,?,?)`)
      .run(
        id,
        opts.perItemCents,
        opts.dailyCents,
        JSON.stringify(opts.merchants.map((m) => m.toLowerCase())),
...
export function evaluateQuote(db: Db, cfg: Config, chain: ChainView, q: QuoteInput): Decision {
  return decide(
    { amountCents: q.amountCents, merchantHost: q.merchantHost.toLowerCase() },
  ...
  return db.tx((t) => {
    ...
    t.raw
      ...
      .run(
        ...
        m.id,
        q.itemName,
        q.merchantHost.toLowerCase(),
        q.productUrl ?? null,
        q.amountCents,
        null,
        "RESERVED",
        d.decision === "NEEDS_HUMAN" ? 0 : 1,
        null,
        new Date(Date.now() + cfg.reservationTtlMs).toISOString(),
        iso(),
        iso(),
      );
    appendAudit(t, {
      purchaseId: id,
      kind: "RESERVED",
      detail: { amountCents: q.amountCents, band: d.decision },
    });
...
export function approvePurchase(db: Db, purchaseId: string) {
  db.tx((t) => {
    t.raw
      .prepare(`UPDATE purchases SET approved=1, updated_at=? WHERE id=?`)
      .run(iso(), purchaseId);
...
function requireState(db: Db, id: string, expected: string[]) {
  ...
  if (!expected.includes(row.state))
    throw new Error(`purchase ${id} is ${row.state}, expected ${expected.join("|")}`);
  return row;
}
...
export function markPaying(db: Db, id: string, finalCents: Cents) {
  db.tx((t) => {
    ...
    t.raw
      .prepare(`UPDATE purchases SET state='PAYING', final_cents=?, updated_at=? WHERE id=?`)
...
export function markIssued(
  ...
  db.tx((t) => {
    ...
    t.raw
      .prepare(`UPDATE purchases SET state='CARD_ISSUED', final_cents=?, updated_at=? WHERE id=?`)
      .run(finalCents, iso(), id);
    t.raw
    ...
    appendAudit(t, {
      purchaseId: id,
      kind: "CARD_ISSUED",
      detail: { finalCents, last4: card.last4 },
    });
  });
}
...
export function markDone(db: Db, id: string, orderRef: string | null) {
  db.tx((t) => {
    ...
    t.raw
      .prepare(`UPDATE purchases SET state='DONE', order_ref=?, updated_at=? WHERE id=?`)
      .run(orderRef, iso(), id);
...
export function markCancelled(db: Db, id: string, reason: string) {
  db.tx((t) => {
    ...
    if (!row) throw new Error(`unknown purchase ${id}`);
    if (row.state === "PAYING")
      throw new Error(`cannot cancel ${id}: PAYING — settlement outcome unknown`);
    if (row.state !== "RESERVED" && row.state !== "CARD_ISSUED")
      throw new Error(
        `cannot cancel ${id}: state is ${row.state}, expected RESERVED or CARD_ISSUED`,
      );
    ...
    if (next === "STRANDED")
      t.raw.prepare(`UPDATE cards SET state='DEAD' WHERE purchase_id=?`).run(id);
...
export function releaseExpired(db: Db): number {
  return db.tx((t) => {
    const rows = t.raw
      .prepare(`SELECT id FROM purchases WHERE state='RESERVED' AND reserved_until < ?`)
      .all(iso()) as any[];
    for (const r of rows) {
      t.raw
        .prepare(`UPDATE purchases SET state='RELEASED', updated_at=? WHERE id=?`)
        .run(iso(), r.id);
      appendAudit(t, {
        purchaseId: r.id,
        kind: "RELEASED",
        detail: { reason: "reservation_expired" },
      });
    }
    return rows.length;
  });
